 ...
 *  net.raphimc.minecraftauth.msa.model.MsaDeviceCode
 ...
 *  org.cloudburstmc.protocol.bedrock.packet.LoginPacket
 *  org.cloudburstmc.protocol.bedrock.packet.ServerToClientHandshakePacket
 *  org.cloudburstmc.protocol.bedrock.util.ChainValidationResult
 *  org.cloudburstmc.protocol.bedrock.util.ChainValidationResult$IdentityData
 ...
 *  org.geysermc.cumulus.response.result.FormResponseResult
 *  org.geysermc.cumulus.response.result.ValidFormResponseResult
 *  org.geysermc.geyser.GeyserImpl
 *  org.geysermc.geyser.network.CodecProcessor
...
public class LoginEncryptionUtils {
    ...
    private static void encryptConnectionWithCert(GeyserSession session, AuthPayload authPayload, String jwt) {
        try {
            ...
            if (authPayload.getAuthType() == AuthType.GUEST) {
                session.disconnect(GeyserLocale.getLocaleStringLog((String)"geyser.network.remote.invalid_xbox_account"));
                return;
            }
            ChainValidationResult result = EncryptionUtils.validatePayload((AuthPayload)authPayload);
            geyser.getLogger().debug("Is player data signed? %s", new Object[]{result.signed()});
            ...
            byte[] clientDataPayload = EncryptionUtils.verifyClientData((String)jwt, (PublicKey)identityPublicKey);
            if (clientDataPayload == null) {
                throw new IllegalStateException("Client data isn't signed by the given chain data");
            }
            BedrockClientData data = (BedrockClientData)JsonUtils.fromJson((byte[])clientDataPayload, BedrockClientData.class);
            ...
            CodecProcessor.updateCodec((UpstreamSession)session.getUpstream(), (String)data.getGameVersion());
            try {
    ...
    public static void buildAndShowMicrosoftCodeWindow(GeyserSession session, MsaDeviceCode msCode) {
        ...
        if (timeout != 0) {
            message.append("\n\n").append("\u00a7r").append(GeyserLocale.getPlayerLocaleString((String)"geyser.auth.login.timeout", (String)session.locale(), (Object[])new Object[]{String.valueOf(timeout)}));
        }
        session.sendForm(((ModalForm.Builder)((ModalForm.Builder)ModalForm.builder().title("%xbox.signin")).content(message.toString()).button1("%gui.done").button2("%menu.disconnect").closedOrInvalidResultHandler(() -> LoginEncryptionUtils.buildAndShowLoginWindow(session))).validResultHandler(response -> {
            if (response.clickedButtonId() == 1) {
                session.disconnect(GeyserLocale.getPlayerLocaleString((String)"geyser.auth.login.form.disconnect", (String)locale));
            }
        }));
    }
        ...
        while (previousIndex < key.length()) {
            ...
            if (endIndex - previousIndex > 1) {
                ...
                if (key.charAt(previousIndex) != '%') {
                    newValue.append(GeyserLocale.getPlayerLocaleString((String)substring, (String)locale));
                } else {
