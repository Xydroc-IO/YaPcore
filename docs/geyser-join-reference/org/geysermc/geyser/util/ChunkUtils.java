 ...
 *  lombok.Generated
 *  org.cloudburstmc.math.GenericMath
 *  org.cloudburstmc.math.vector.Vector2i
 ...
 *  org.cloudburstmc.protocol.bedrock.packet.LevelChunkPacket
 *  org.cloudburstmc.protocol.bedrock.packet.NetworkChunkPublisherUpdatePacket
 *  org.cloudburstmc.protocol.bedrock.packet.UpdateBlockPacket
 ...
 *  org.geysermc.geyser.level.block.Blocks
 ...
 *  org.geysermc.geyser.level.chunk.bitarray.BitArray
 *  org.geysermc.geyser.level.chunk.bitarray.SingletonBitArray
 *  org.geysermc.geyser.platform.spigot.shaded.it.unimi.dsi.fastutil.ints.IntLists
...
public final class ChunkUtils {
    ...
    public static void updateChunkPosition(GeyserSession session, Vector3i position) {
        ...
        Vector2i newChunkPos = Vector2i.from((int)(position.getX() >> 4), (int)(position.getZ() >> 4));
        if (chunkPos == null || !chunkPos.equals(newChunkPos)) {
            ...
            session.sendUpstreamPacket((BedrockPacket)chunkPublisherUpdatePacket);
            session.setLastChunkPosition(newChunkPos);
        }
    }

    public static int squareToCircle(int renderDistance) {
        return GenericMath.ceil((double)((double)(renderDistance + 1) * MathUtils.SQRT_OF_TWO));
    }

    public static void updateBlock(GeyserSession session, int blockState, Vector3i position) {
        ChunkUtils.updateBlockClientSide(session, BlockState.of((int)blockState), position);
    ...
    public static void updateBlockClientSide(GeyserSession session, BlockState blockState, Vector3i position) {
        ItemFrameEntity itemFrameEntity = ItemFrameEntity.getItemFrameEntity((GeyserSession)session, (Vector3i)position);
    ...
    public static void sendEmptyChunk(GeyserSession session, int chunkX, int chunkZ, boolean forceUpdate) {
        ...
        session.sendUpstreamPacket((BedrockPacket)data);
        if (forceUpdate) {
            Vector3i pos = Vector3i.from((int)(chunkX << 4), (int)80, (int)(chunkZ << 4));
            UpdateBlockPacket blockPacket = new UpdateBlockPacket();
            blockPacket.setBlockPosition(pos);
            blockPacket.setDataLayer(0);
            blockPacket.setDefinition((BlockDefinition)session.getBlockMappings().getBedrockBlock(1));
    ...
    public static void loadDimension(GeyserSession session) {
        ...
        if (SHOW_CHUNK_HEIGHT_WARNING_LOGS && (minY < bedrockDimension.minY() || bedrockDimension.doUpperHeightWarn() && maxY > bedrockDimension.maxY())) {
            session.getGeyser().getLogger().warning(GeyserLocale.getLocaleStringLog((String)"geyser.network.translator.chunk.out_of_bounds", (Object[])new Object[]{String.valueOf(bedrockDimension.minY()), String.valueOf(bedrockDimension.maxY()), session.getRegistryCache().registry(JavaRegistries.DIMENSION_TYPE).byValue((Object)session.getDimensionType())}));
        }
    ...
    static {
        ...
        try {
            BlockStorage blockStorage = new BlockStorage((BitArray)SingletonBitArray.INSTANCE, IntLists.singleton((int)0));
