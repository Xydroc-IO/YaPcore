...
If you want the academic / engineer version (full plugin catalog, data plane, smokes), use the [technical whitepaper](YAPCORE_WHITEPAPER.md).
...
6. **Shipped YaP plugins** — CORE+NETWORK by default; GAMEPLAY opt-in — so you are not piecing together LuckPerms + EssentialsX + … for a normal survival/network.
...
- Resource packs can be served over HTTP; the default pack is Faithful + skies/water. Ops use a browser dashboard on port **8080** (including a **Custom commands** tab).
