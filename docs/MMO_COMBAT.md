...
Mob defence/strength derive from mob max health. Player stats come from
`SkillService` (attack, strength, defence, hitpoints) plus gear from
`items.yml`.
...
- [PERMISSIONS.md](PERMISSIONS.md) — `yapcombat.*` nodes
