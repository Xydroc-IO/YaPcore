...
| **Heavy / prod launcher** | `12288` (`./scripts/lifecycle/start-prod.sh`, override with `--heap-gb=N`) | ~6 GB | Busy Folia, farms, large plugin surface |
...
| **Crop growth NMS / fluid tick** | E2-NMS | `gameplay.crop-growth-nms` + YaP-Folia `YapEncyclopediaHooks` (`vendor/folia/patches/0025-yap-encyclopedia-hooks.patch`) — rebuild `./scripts/folia/build-yap-folia.sh`; soak-gate before enabling |
...
./scripts/lifecycle/yapctl soak-compat   # PASS before production enable
