...
We are **not** “stock Folia with a sticker.” The game jar is **YaP-Folia** (`lib/yap-folia-26.2.jar`, `folia-jar-source=build`). Stock Fill Folia is fallback only. See [FOLIA_FORK.md](../folia/FOLIA_FORK.md).
...
| Typical network stack | LP, Essentials, WE, Velocity, Geyser, … | Same glue work | **Shipped natives** — [PLUGIN_COMPAT_MATRIX.md](../plugins/PLUGIN_COMPAT_MATRIX.md) |
...
4. **YaP Link** — first-party native Velocity-class proxy (`0.6.0-phase6`) — [YAP_LINK.md](../network/YAP_LINK.md) · [YAP_LINK_NATIVE.md](../network/YAP_LINK_NATIVE.md).
5. **Phase 3 Paper spatial (legacy):** defaults **off**; Folia path has **no** Phase 3 spatial tick.
6. **Dual-stack:** first-party Via\* + Geyser-class code — no Via\*/Geyser jars — [VIA_GEYSER_PARITY.md](../protocol/VIA_GEYSER_PARITY.md).
...
9. **Vehicles:** real chassis / fleet / fuel — GAMEPLAY opt-in — [VEHICLES.md](../plugins/VEHICLES.md).
...
Chassis channel map: [YAPENGINE_16THREAD.md](../performance/YAPENGINE_16THREAD.md). Fork patches: [FOLIA_FORK.md](../folia/FOLIA_FORK.md).
...
**GNU GPLv3** (or later) — [LICENSING.md](../start/LICENSING.md) · root [`LICENSE`](../../LICENSE).
