...
**M5–M7 (extended):** Bedrock MMO UI (M5), ability engine + 230+ combat abilities (M6), AoE/homing/conditions + spell icons (M7).
See `docs/MMO_BEDROCK_UI.md` and `docs/MMO_ABILITIES.md`.
...
| Docs | `docs/MMO_SKILLS.md` — skill list, XP sources, permissions |
...
```
Implement YaP MMO Phase M1 per docs/MMO_PHASES.md on top of merged M0.
...
```
Implement YaP MMO Phase M2 per docs/MMO_PHASES.md.
...
```
Implement YaP MMO Phase M3 per docs/MMO_PHASES.md.
...
```
Implement YaP MMO Phase M4 per docs/MMO_PHASES.md.
...
- `docs/FULL_RUNDOWN.md` — MMO section
- `docs/PERMISSIONS.md` — `yapskills.*`, `yapcombat.*`, `yapcraft.*`, `yapmmo.*`, `yapabilities.*`
- `gradle/yap-product.gradle.kts` — gameplay install includes all MMO jars
- `gradle/yap-release.gradle.kts` — release artifact list
- `docs/MMO_PHASES.md` — milestone status
