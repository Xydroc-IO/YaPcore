...
/** Dashboard routes: chat, moderation, perms, playerdata. */
final class DashboardGameplaySocialOpsApi {

    private final YaPcoreServer server;
    private final DashboardAuth auth;

    DashboardGameplaySocialOpsApi(YaPcoreServer server, DashboardAuth auth) {
        this.server = server;
        this.auth = auth;
    }

    void apiChat(HttpExchange ex) throws IOException {
    ...
    void apiModeration(HttpExchange ex) throws IOException {
        if (!auth.requireAuth(ex)) {
            return;
        }
        Path root = server.getRootDir();
    ...
    void apiPerms(HttpExchange ex) throws IOException {
        if (!auth.requireAuth(ex)) {
            return;
        }
        Path root = server.getRootDir();
    ...
    void apiPlayerdata(HttpExchange ex) throws IOException {
        if (!auth.requireAuth(ex)) {
            return;
        }
        Path root = server.getRootDir();
