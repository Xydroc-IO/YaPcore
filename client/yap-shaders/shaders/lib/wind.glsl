...
// Wind-driven foliage sway:
//  - Global wind direction (sway goes with the wind)
//  - Speed from weather (clear ≈ still; rain/thunder = more)
//  - Species profiles for leaf canopies (birch flexible, spruce stiff, …)
//  - Height bend (lower leaves less, top leads)
//
// Logs / wood / stems intentionally do NOT sway — a log house must stay still.
// block.properties: 10011–10015 leaves · 10002 grass · 10003 vines · 10025 azalea bush
...
// Leaf canopy species → flex + heightRef
void yapSpeciesProfile(float entityId, out float flex, out float heightRef) {
    flex = 1.0;
    heightRef = 9.0;

    // Flexible: birch / cherry / pale oak leaves
    if (abs(entityId - 10011.0) < 0.5) { flex = 1.30; heightRef = 7.0; return; }

    // Medium: oak / acacia / mangrove leaves
    if (abs(entityId - 10012.0) < 0.5) { flex = 1.00; heightRef = 9.0; return; }

    // Stiff: spruce / dark oak leaves
    if (abs(entityId - 10013.0) < 0.5) { flex = 0.72; heightRef = 11.0; return; }

    // Tall jungle leaves
    if (abs(entityId - 10014.0) < 0.5) { flex = 1.15; heightRef = 14.0; return; }

    // Soft azalea leaves
    if (abs(entityId - 10015.0) < 0.5) { flex = 1.35; heightRef = 5.0; return; }
}

vec3 yapLeafSway(vec3 wp, vec3 midOff, float t, float entityId) {
    float flex, heightRef;
    yapSpeciesProfile(entityId, flex, heightRef);
    ...
    float bend = yapTreeBendAmt(wp.y, heightRef) * flex;
...
// Leaves / canopy only — never logs, wood, or stems (builds use those).
bool yapIsLeafId(float id) {
    return abs(id - 10011.0) < 0.5 || abs(id - 10012.0) < 0.5
...
vec3 yapApplyWind(vec3 wp, float entityId, float tip, vec3 midOff) {
    ...
    if (yapIsLeafId(entityId)) {
        return wp + yapLeafSway(wp, midOff, frameTimeCounter, entityId);
    }
    if (abs(entityId - 10002.0) < 0.5 || abs(entityId - 10025.0) < 0.5) {
