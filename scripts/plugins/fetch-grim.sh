...
#
# Usage:
#   ./scripts/plugins/fetch-grim.sh                    # → plugins/grim.jar (active)
#   ./scripts/plugins/fetch-grim.sh --disabled         # → plugins/grim.jar.disabled (default on setup)
#   ./scripts/plugins/fetch-grim.sh --disabled --root /path/to/home
set -euo pipefail
...
if [ "$QUIET" -eq 0 ]; then
  ...
  if [ "$DISABLED" -eq 1 ]; then
    echo "Grim is disabled until: ./scripts/plugins/grim-ac.sh enable"
  else
