...
YaPGuard alone is not gold-standard AC — see docs/ops/GRIM.md.
...
case "$CMD" in
  status)
    ...
    echo "  Run: ./scripts/setup/seed-defaults.sh  (or ./scripts/plugins/fetch-grim.sh --disabled)"
    exit 1
    ;;
  enable)
    if [ -f "$ACTIVE" ]; then
    ...
    if [ ! -f "$DISABLED" ]; then
      echo "Missing $DISABLED — run ./scripts/setup/seed-defaults.sh first" >&2
      exit 1
    fi
    mv "$DISABLED" "$ACTIVE"
