...
#   ./scripts/setup/seed-defaults.sh --root /path/to/yap-home
#   ./scripts/setup/seed-defaults.sh --force-missing-only   # default behavior
set -eu
...
if [ "${YAP_SKIP_OPTIONAL_FETCH:-}" != "1" ] && [ -x "$SCRIPT_DIR/fetch-grim.sh" ]; then
  if [ ! -f "$ROOT/plugins/grim.jar" ] && [ ! -f "$ROOT/plugins/grim.jar.disabled" ]; then
    echo "Fetching Grim AC (top-tier, disabled until ./scripts/plugins/grim-ac.sh enable) …"
    if "$SCRIPT_DIR/fetch-grim.sh" --disabled --root "$ROOT" --quiet; then
      echo "  grim.jar.disabled ready"
    else
      echo "  Grim fetch skipped (offline or Modrinth unavailable) — run ./scripts/plugins/fetch-grim.sh --disabled later"
    fi
  fi
fi

echo "Tip: ./configure-db.sh --server-id lobby  # after MariaDB is healthy"
echo "Tip: ./scripts/plugins/grim-ac.sh enable          # when you want Grim AC active"
