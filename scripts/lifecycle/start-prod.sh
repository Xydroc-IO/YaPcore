...
#   YAPCORE_PROD_HEAP_GB=16 ./scripts/lifecycle/start-prod.sh
#
# Usage: ./scripts/lifecycle/start-prod.sh [--nogui|--gui] [--fg] [--heap-gb N]
...
if yap_is_running; then
  echo "YaPcore is already running (pid $(yap_find_product_pid)). Use scripts/lifecycle/stop.sh first." >&2
  exit 1
fi
