...
# Usage: ./scripts/lifecycle/start.sh [--nogui|--gui] [--fg]
...
if yap_is_running; then
  ...
  echo "Use scripts/lifecycle/stop.sh first. (Bench runs under bench/workdir-* do not block start/gui.)" >&2
  exit 1
fi
...
if [ "$FOREGROUND" -eq 1 ] || [ "$MODE" = "gui" ]; then
  ...
  echo "Stop with: $ROOT/scripts/lifecycle/stop.sh"
fi
