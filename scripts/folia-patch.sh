...
case "$PHASE" in
  ...
  --list|list)
    ...
    echo "post:"; ls -1 "$PATCH_DIR"/000[1-9]-*.patch "$PATCH_DIR"/001*.patch "$PATCH_DIR"/002*.patch "$PATCH_DIR"/003*.patch "$PATCH_DIR"/004*.patch "$PATCH_DIR"/005*.patch "$PATCH_DIR"/006*.patch 2>/dev/null || true
    exit 0
    ;;
  *)
...
if [ "$PHASE" = "pre" ]; then
  mapfile -t PATCHES < <(find "$PATCH_DIR" -maxdepth 1 -type f -name '0000-*.patch' | sort)
    ...
    mapfile -t PATCHES < <(find "$PATCH_DIR" -maxdepth 1 -type f \( -name '000[1-9]-*.patch' -o -name '001*.patch' -o -name '002*.patch' -o -name '003*.patch' -o -name '004*.patch' -o -name '005*.patch' -o -name '006*.patch' \) | sort)
fi
