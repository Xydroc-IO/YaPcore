 ...
 *
 * YAP_BOT_NO_DIG=1 — skip block dig (fullcite cite path). Digging caves explodes
 * chunk load mid-sample and dilutes MSPT deltas; both competitors use the same flag.
 */
...
const noDig = process.env.YAP_BOT_NO_DIG === '1'
  || process.env.YAP_BOT_NO_DIG === 'true'
...
function behave(bot, id) {
  ...
  const tick = () => {
    ...
    bot.setControlState('sprint', !noDig && !light && id % 3 === 0)
    bot.setControlState('jump', !noDig && !light && Math.random() < 0.05)
    ...
    if (!noDig && !light && Math.random() < 0.02) {
