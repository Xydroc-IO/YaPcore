...
if [ "$NEEDS_BOTS" = "1" ]; then
  ...
  # minecraft-data indexes 26.2 but ships no pc/26.2 schemas yet — patch clones 26.1 → 26.2.
  if [ -x "$BOTS_DIR/patch-minecraft-data.sh" ]; then
    echo "Patching Mineflayer stack for protocol 26.2…"
    "$BOTS_DIR/patch-minecraft-data.sh"
  else
    echo "ERROR: missing $BOTS_DIR/patch-minecraft-data.sh (required for 26.2 bot joins)" >&2
    exit 2
  fi
else
...
BUDGET_MSPT_THRESHOLD="${YAP_FOLIA_BUDGET_MSPT_THRESHOLD:-}"
if [ "$SCENARIO" = "spawncollapse" ] && [ -z "$ENTITY_TICK_BUDGET" ]; then
  ENTITY_TICK_BUDGET="${YAP_FOLIA_ENTITY_TICK_BUDGET:-400}"
fi
# Population / marketing scenarios: default to **ship** smart knobs (product profile).
# Override with YAP_BENCH_CITE_BASELINE=1 for async+hopper-only A/B.
case "$SCENARIO" in
  heavypop|fullcite|highpop)
    if [ "${YAP_BENCH_CITE_BASELINE:-}" = "1" ] || [ "${YAP_BENCH_CITE_BASELINE:-}" = "true" ]; then
...
# Ship hopper budget on YaP cite path (stock Folia ignores unknown -D).
if [ -z "$HOPPER_TICK_BUDGET" ]; then
  HOPPER_TICK_BUDGET="${YAP_FOLIA_HOPPER_TICK_BUDGET:-64}"
fi
...
bench_jvm_extra() {
  ...
  if [ "$yap_knobs" = "1" ]; then
    # Always emit explicit values so Folia JVM cannot fall through to unset defaults.
    extra+=(-Dyap.folia.entity-tick-budget="${ENTITY_TICK_BUDGET:-0}")
    extra+=(-Dyap.folia.microtick-budget-ms="${MICROTICK_BUDGET:-0}")
    extra+=(-Dyap.folia.async-chunk-save="${ASYNC_CHUNK_SAVE:-true}")
    ...
    extra+=(-Dyap.bench.knob_hopper_tick_budget="${HOPPER_TICK_BUDGET:-64}")
...
start_bots() {
  ...
  (
    ...
    export YAP_BOT_NO_DIG="${YAP_BOT_NO_DIG:-}"
    export YAP_BOT_ACTIVE="${YAP_BOT_ACTIVE:-}"
...
folia-async-chunk-save=${ASYNC_CHUNK_SAVE:-true}
folia-entity-tick-budget=${ENTITY_TICK_BUDGET:-0}
folia-microtick-budget-ms=${MICROTICK_BUDGET:-0}
