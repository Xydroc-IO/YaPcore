...
#   ./scripts/db/configure-playerdata.sh --root /path/to/yap-home --server-id lobby
set -eu
...
PLUGIN_DIR="$TARGET_ROOT/plugins/YaPPlayerData"
...
# Portable patch without requiring yq
...
# Also configure shared YaPDB under the same target root
...
echo "Tip: ./scripts/db/configure-db.sh --root <home>  (shared pool + optional --server-id)"
