...
# Usage: ./scripts/folia/smoke-folia.sh [seconds]
set -euo pipefail
...
if [ "$FOLIA_SRC" = "build" ]; then
  if [ ! -f "$YAP_JAR_CAND" ]; then
    echo "folia-jar-source=build requires $YAP_JAR_CAND — run ./scripts/folia/build-yap-folia.sh" >&2
    exit 1
  fi
...
folia-async-chunk-save=${YAP_FOLIA_ASYNC_CHUNK_SAVE:-true}
folia-entity-tick-budget=${YAP_FOLIA_ENTITY_TICK_BUDGET:-400}
folia-budget-mspt-threshold=${YAP_FOLIA_BUDGET_MSPT_THRESHOLD:-12}
folia-entity-tick-max-deferred=${YAP_FOLIA_ENTITY_TICK_MAX_DEFERRED:-40}
...
folia-subregion-partition=${YAP_FOLIA_SUBREGION_PARTITION:-true}
folia-subregion-shards=${YAP_FOLIA_SUBREGION_SHARDS:-2}
folia-subregion-mspt-threshold=${YAP_FOLIA_SUBREGION_MSPT_THRESHOLD:-20}
folia-subregion-mspt-clear=${YAP_FOLIA_SUBREGION_MSPT_CLEAR:-16}
...
folia-subregion-coalesce-min-wall-ms=${YAP_FOLIA_SUBREGION_COALESCE_MIN_WALL_MS:-30000}
folia-subregion-partition-delay-ticks=${YAP_FOLIA_SUBREGION_PARTITION_DELAY_TICKS:-600}
