...
# Usage:
#   ./scripts/folia/soak-yap-folia.sh compat              # boot + API smoke (~5–15 min)
#   ./scripts/folia/soak-yap-folia.sh perf [minutes]      # load + heap samples (default 30)
#   ./scripts/folia/soak-yap-folia.sh long [hours]        # slope soak (default 12, min 8 via YAP_SOAK_HOURS)
#
# Env:
...
# Byte offset into the locked Folia log at soak start — only NEW lines FAIL.
SOAK_LOG_FILE=""
SOAK_LOG_OFFSET=0

mark_soak_log_origin() {
  local pid="${1:-$(folia_pid || true)}"
  local cwd=""
  SOAK_LOG_FILE=""
  if [ -n "$pid" ] && [ -d "/proc/$pid" ]; then
    cwd="$(readlink -f "/proc/$pid/cwd" 2>/dev/null || true)"
  fi
  if [ -n "$cwd" ] && [ -f "$cwd/logs/latest.log" ]; then
    SOAK_LOG_FILE="$cwd/logs/latest.log"
  elif [ -f "$ROOT/folia-kernel/logs/latest.log" ]; then
    SOAK_LOG_FILE="$ROOT/folia-kernel/logs/latest.log"
  fi
  if [ -n "$SOAK_LOG_FILE" ]; then
    SOAK_LOG_OFFSET="$(wc -c <"$SOAK_LOG_FILE" | tr -d ' ')"
  else
    SOAK_LOG_OFFSET=0
  fi
  echo "Soak log origin offset=${SOAK_LOG_OFFSET} (${SOAK_LOG_FILE:-missing})" | tee -a "$REPORT"
}

scan_logs_for_bad() {
  local log="$SOAK_LOG_FILE"
  local crash_dir="$ROOT/logs/crashes"
  local bad=0
  if [ -z "$log" ] || [ ! -f "$log" ]; then
    echo "FAIL: no Folia log to scan" | tee -a "$REPORT"
    return 1
  fi
  ...
  if printf '%s\n' "$recent" | grep -E 'OutOfMemoryError|owning region|Cannot modify|TickThread' >/dev/null 2>&1; then
    echo "FAIL: suspicious lines in $log since soak start" | tee -a "$REPORT"
...
run_compat() {
  ...
  if [ -x "$ROOT/scripts/folia/disasters-mem-smoke.py" ] || [ -f "$ROOT/scripts/folia/disasters-mem-smoke.py" ]; then
    python3 "$ROOT/scripts/folia/disasters-mem-smoke.py" 2>&1 | tee -a "$REPORT" || {
...
run_perf() {
  ...
  if [ -f "$ROOT/scripts/folia/disasters-mem-smoke.py" ]; then
    python3 "$ROOT/scripts/folia/disasters-mem-smoke.py" 2>&1 | tee -a "$REPORT" || return 1
  fi
  scan_logs_for_bad || return 1
  slope_fail || return 1
