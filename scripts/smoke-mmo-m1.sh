#!/usr/bin/env bash
# M1 smoke: M0 tests + skill pack tests + yap-skills compile + optional Folia boot.
# Usage: ./scripts/smoke-mmo-m1.sh
set -euo pipefail
...
echo "== M1 build + install =="
gradle :yap-mmo-api:jar :skills-plugin:installIntoPlugins :tab-plugin:installIntoPlugins --no-daemon -q
...
if [ "${SKIP_LIVE:-0}" = "1" ]; then
  ...
  echo "M1 smoke PASS (compile + unit)"
  exit 0
fi
...
WORK="$ROOT/bench/workdir-mmo-m1-smoke"
rm -rf "$WORK"
mkdir -p "$WORK/config" "$WORK/lib" "$WORK/plugins" "$WORK/logs"
/bin/cp -f "$ROOT/lib/folia-${VER}.jar" "$WORK/lib/"

for j in yap-folia-bridge.jar yap-db.jar yap-skills.jar yap-tab.jar yap-placeholderapi.jar; do
  if [ -f "$ROOT/plugins/$j" ]; then
    /bin/cp -f "$ROOT/plugins/$j" "$WORK/plugins/"
  fi
done

"$ROOT/scripts/db/ensure-db.sh" --root "$WORK" --server-id smoke-mmo-m1

PORT=25578
cat >"$WORK/config/server.properties" <<EOF
server-name=YaP-MMO-M1-Smoke
bind-host=127.0.0.1
port=${PORT}
max-players=10
ram-mb=2048
...
echo "M1 smoke PASS (live boot + YaPSkills ready)"
