...
public final class StackerTasks {
    ...
    public void start() {
        ...
        wanderTask = YapSched.globalTimer(plugin.bukkit(), this::wanderMerge, wander, wander);
        long aura = stacks.config().killAuraIntervalTicks();
        auraTask = YapSched.globalTimer(plugin.bukkit(), this::auraPulse, aura, aura);
    }
    ...
    private void wanderMerge() {
        ...
        for (World world : Bukkit.getWorlds()) {
            ...
            for (LivingEntity living : mobs) {
                ...
                YapSched.entity(plugin.bukkit(), living, () -> mergeLivingOnEntityThread(living, budget));
            }
            if (items.enabled()) {
                ...
                for (Item item : drops) {
                    ...
                    YapSched.entity(plugin.bukkit(), item, () -> mergeItemOnEntityThread(item, budget));
                }
            }
        }
    }
    ...
    private void auraPulse() {
        for (Player player : Bukkit.getOnlinePlayers()) {
            YapSched.entity(plugin.bukkit(), player, () -> tools.pulseAura(player));
        }
    }
}
