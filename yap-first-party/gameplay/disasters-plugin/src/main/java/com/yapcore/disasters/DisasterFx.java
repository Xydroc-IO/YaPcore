...
/** Folia-safe per-tick disaster FX. */
final class DisasterFx {

    static final int MAX_UNDO_TASKS = 256;

    private final DisastersPlugin plugin;
    private final Random random;

    DisasterFx(DisastersPlugin plugin, Random random) {
        this.plugin = plugin;
        this.random = random;
    }

    void tick(DisasterActive effect, World world) {
        switch (effect.type) {
    ...
    private void tickTornado(World world, DisasterActive effect) {
    ...
    private void tickVolcano(World world, DisasterActive effect) {
    ...
    private void tickBlizzard(World world, DisasterActive effect) {
        DisastersConfig cfg = plugin.config();
    ...
    private void tickDrought(World world, DisasterActive effect) {
        DisastersConfig cfg = plugin.config();
    ...
    private void tickMeteor(World world, DisasterActive effect) {
        DisastersConfig cfg = plugin.config();
    ...
    private void tickTsunami(World world, DisasterActive effect) {
        DisastersConfig cfg = plugin.config();
    ...
    private boolean scheduleBlockUndo(DisasterActive effect, Location loc, long delayTicks, Runnable regionWork) {
