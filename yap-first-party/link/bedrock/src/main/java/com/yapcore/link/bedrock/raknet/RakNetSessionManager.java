...
public final class RakNetSessionManager {
    ...
    public final class RakNetPeer {
        ...
        /**
         * UDP Channel that received this peer's datagrams. Game replies MUST go out on the same
         * socket (source port). Using the wrong listen (e.g. :25565 while phone used :19132)
         * drops NetworkSettings and leaves the client stuck on RequestNetworkSettings.
         */
        ...
        final java.util.concurrent.atomic.AtomicInteger encryptAppliedLogs =
        ...
        public void setListenChannel(Channel channel) {
            if (channel != null) {
                    ...
                    try {
                        ...
                        if (inner == RakNetReliability.ID_CONNECTION_REQUEST) {
                            ...
                            if (peer.gameCompressionHeader() && afterFe.isReadable()) {
                                ...
                                if (method == 0) {
                                    // ZLIB — try zlib-wrapped then raw deflate
                                    batch = RakNetSessionCodec.inflateRaw(afterFe);
                                    if (batch == null) {
                                        int headLen = Math.min(8, afterFe.readableBytes());
                                        StringBuilder hex = new StringBuilder();
                                        for (int i = 0; i < headLen; i++) {
                                            hex.append(String.format("%02x", afterFe.getUnsignedByte(afterFe.readerIndex() + i)));
                                        }
                                    ...
                                    // SNAPPY (some BE clients ignore ZLIB-only NetworkSettings)
                                    batch = RakNetSessionCodec.inflateSnappy(afterFe);
                                    if (batch == null) {
                                    ...
                                    String peekIds = RakNetSessionCodec.peekBatchPacketIds(afterFe, 4);
    ...
    public List<ByteBuf> encapsulateGameDatagrams(RakNetPeer peer, ByteBuf gameBatch) {
        return RakNetSessionCodec.encapsulateGameDatagrams(peer, gameBatch);
    }

    public ByteBuf encapsulateGame(RakNetPeer peer, ByteBuf gameBatch) {
        return RakNetSessionCodec.encapsulateGame(peer, gameBatch);
    }
