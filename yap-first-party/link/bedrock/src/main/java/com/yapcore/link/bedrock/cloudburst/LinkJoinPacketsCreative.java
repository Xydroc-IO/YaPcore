...
/**
 * CreativeContent packet factories (split from {@link LinkJoinPackets}).
 */
final class LinkJoinPacketsCreative {

    private static final Logger LOG = Logger.getLogger("YaP.Link.Bedrock");

    private LinkJoinPacketsCreative() {}

    /**
     * Empty CreativeContent (~4B). Proven on JOIN_OK probe 015924; full catalog caused IC-90
     * on 020914. Prefer stability over creative-inventory UI completeness.
     */
    ...
    /**
     * Full CreativeContent from {@code creative_items.26_40.json} resolved against the session
     * item palette. Do not use on the join critical path until wire format is validated —
     * 41KB full CreativeContent + PlayerList preceded IC-90 (probe 020914).
     */
    static CreativeContentPacket creativeContentFull(LinkCloudburstCodecs.Session session) {
        return CreativeContentCache.packetFor(session);
    }

    private static final class CreativeContentCache {
        private static final Map<String, CreativeContentPacket> BY_BAND = new ConcurrentHashMap<>();

        static CreativeContentPacket packetFor(LinkCloudburstCodecs.Session session) {
            String band = session != null && session.palettes() != null
                    ? session.palettes().band() : "band_26_40";
        ...
        private static CreativeContentPacket build(LinkCloudburstCodecs.Session session) {
            CreativeContentPacket packet = new CreativeContentPacket();
            String path = "protocol/bedrock/cloudburst/creative_items.26_40.json";
            try (InputStream in = CreativeContentCache.class.getClassLoader().getResourceAsStream(path)) {
                if (in == null) {
                ...
                LOG.info("BE CreativeContent loaded groups=" + packet.getGroups().size()
                        + " items=" + added + " band="
                        + (session != null && session.palettes() != null ? session.palettes().band() : "?"));
            } catch (Exception e) {
