        ...
        try {
            switch (packetId) {
                ...
                case JavaPlayWire.CB_SET_ENTITY_DATA -> {
                    int entityId = McCodec.readVarInt(buf);
                    // Best-effort: extract custom name (meta index 2, serializer OPTIONAL_COMPONENT=6).
                    ...
                    try {
                        while (buf.isReadable()) {
                            ...
                            if (index == 2 && (serializer == 5 || serializer == 6)) {
                                // COMPONENT or OPTIONAL_COMPONENT
                                ...
                                // BOOLEAN in some mappings — try readBoolean after varint type
