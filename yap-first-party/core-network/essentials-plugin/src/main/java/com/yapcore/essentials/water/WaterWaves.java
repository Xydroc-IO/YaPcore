package com.yapcore.essentials.water;
...
public final class WaterWaves {
    ...
    private final Supplier<EssentialsConfig> config;
    private long tick;

    public WaterWaves(JavaPlugin plugin, Supplier<EssentialsConfig> config) {
        this.plugin = plugin;
        this.config = config;
    }

    public void tick(Iterable<? extends Player> players) {
        EssentialsConfig cfg = config.get();
        if (cfg == null || !cfg.waterWavesEnabled()) {
            return;
        }
        tick++;
        double amp = cfg.waterWaveAmplitude();
        double speed = cfg.waterWaveSpeed();
    ...
    public void splash(LivingEntity entity) {
        EssentialsConfig cfg = config.get();
        if (cfg == null || !cfg.waterWavesEnabled() || !cfg.waterSplashEnabled()) {
            return;
        }
