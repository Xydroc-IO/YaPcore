...
public final class EnchantCatalog {
    ...
    public static String idByLabel(String label) {
        ...
        String plain = label.replace("▶ ", "").trim();
        // Strip trailing roman / level
        int space = plain.lastIndexOf(' ');
        if (space > 0) {
            String tail = plain.substring(space + 1);
            if (tail.matches("M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})")
                    || tail.matches("\\d+")) {
                plain = plain.substring(0, space).trim();
            }
        }
        for (Info info : ALL) {
            if (info.label().equalsIgnoreCase(plain) || info.id().equalsIgnoreCase(plain)) {
                return info.id();
            }
        }
        return null;
