 ...
 * Money moves through YaPPlayerData {@link PlayerDataService} when available.
 */
public final class TaxService {
    ...
    public TaxService(JavaPlugin plugin, ClaimsConfig config, ClaimService claims) {
        this.plugin = plugin;
        this.config = config;
        this.claims = claims;
    ...
    private void autoAbandon(Claim claim) {
        try {
    ...
    public String payTax(Player player) throws SQLException {
        ...
        PlayerDataService eco = PlayerDataServiceProvider.find().orElse(null);
        if (eco == null) {
            return "§cYaPPlayerData economy required to pay claim tax.";
        }
        ...
        if (eco.withdraw(player.getUniqueId(), due).isEmpty()) {
            return "§cCould not withdraw tax (economy off or insufficient funds).";
        }
